// Script to check for existing property units and add new ones if needed
import { db } from '../db';
...
async function main() {
  try {
    ...
    const property = await db.select().from(properties).where(eq(properties.id, 2)).execute();
    ...
    const existingUnits = await db.select().from(propertyUnits).where(eq(propertyUnits.propertyId, 2)).execute();
    ...
    if (existingUnits.length === 0) {
      ...
      const newUnits = await db.select().from(propertyUnits).where(eq(propertyUnits.propertyId, 2)).execute();
      console.log('New units:', newUnits);
      
      // Update the unit count on the property
      await db.update(properties)
        .set({ unitCount: sampleUnits.length })
        .where(eq(properties.id, 2));
      
      console.log(`Updated property ID 2 with unit count: ${sampleUnits.length}`);
    }
